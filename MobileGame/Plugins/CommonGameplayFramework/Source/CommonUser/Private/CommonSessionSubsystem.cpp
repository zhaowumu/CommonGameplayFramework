...
void UCommonSessionSubsystem::HostSession(APlayerController* HostingPlayer, UCommonSession_HostSessionRequest* Request)
{
	...
	{
		/**
		 * 如果请求指定的是 离线模式（ECommonSessionOnlineMode::Offline）
		 * 会首先检查当前世界是否处于客户端模式（NM_Client）。如果是客户端模式，系统会报错。
		 * 因为客户端不能主持一个离线游戏（除了客户端的其他模式都可以主持）。
		 */
		...
		else
		{
			 ...
			 * 如果是非客户端，则会调用 ServerTravel 让服务器直接切换到指定的匹配地图/关卡（Request->ConstructTravelURL() 返回的 URL）。
			 */
	...
	{
		// 如果请求指定的是 在线模式，则会调用 CreateOnlineSessionInternal 来创建一个在线游戏会话。这里会使用传入的 LocalPlayer 和 Request 进一步处理会话的创建。
		CreateOnlineSessionInternal(LocalPlayer, Request);
	}

	// 通知更新：最后，无论是在线还是离线会话，都会通过 NotifySessionInformationUpdated 通知系统会话的状态已经更新。在这里，它表示会话已经进入游戏状态
...
void UCommonSessionSubsystem::CreateOnlineSessionInternalOSSv2(ULocalPlayer* LocalPlayer, UCommonSession_HostSessionRequest* Request)
{
	...
	// 获取 OnlineServices 和 Lobbies 接口
